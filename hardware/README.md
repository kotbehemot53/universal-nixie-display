...
Standard-library 3D models are referenced as `${KICAD6_3DMODEL_DIR}/<lib>.3dshapes/<name>.step`. KiCad 7+
maps the old versioned variable to the current install automatically. The references were originally
`.wrl`; KiCad 9+ ships only `.step` files and does *not* fall back from `.wrl` to `.step`, so every
generic part rendered as missing until the extensions were rewritten. Five models were dropped or renamed
upstream and still show as missing in the 3D viewer, which is cosmetic:

| Model                                                             | Board                   |
|-------------------------------------------------------------------|-------------------------|
| `Potentiometer_THT/Potentiometer_Bourns_3386F_Vertical` (9x)      | nixie-board             |
| `Button_Switch_THT/SW_TH_Tactile_Omron_B3F-10xx`                  | vfd-board               |
| `Module/Raspberry_Pi_Zero_Socketed_THT_FaceDown_MountingHoles`    | vfd-board, pi-hat       |
| `Connector_USB/USB_Micro-B_Amphenol_10103594-0001LF_Horizontal`   | pi-hat                  |
| `Connector_Wire/SolderWire-0.75sqmm_1x02_P4.8mm_D1.25mm_OD2.3mm`  | pi-hat                  |

Vendored SamacSys models live in `lib/SamacSys_Parts.3dshapes/` and are referenced via `${KIPRJMOD}`.
